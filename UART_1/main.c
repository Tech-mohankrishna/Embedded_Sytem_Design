...
int main()
	...
	while(1)
	{
		while (LPC_UART0->LSR&0xFF==0X20); // Receive FLAG: check bit 0 of LSR file 
		
		
		{
		LPC_UART0->THR=str1[i]; // transmitting string to THR Register

		while (LPC_UART0->LSR&0xFF==0X20); // waiting till string is transmitted. chrcks 5th bit of LSR, 5th flag bit = 1: complete 
																			// can also be written as while (LPC_UART0->LSR&0x20!=0X20);
		delay(100000);
		}
	}
}
